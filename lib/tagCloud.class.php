...
function tagCloud ($KA,$R=3,$IPR=5,$TC="#0000FF",$FT="Arial",$MinFS=10,$MaxFS=20,$SPBI=5,$LUP='http://localhost/tests/TagCloud/mylink.php?name=',$ANTP=1,$BC="#FFFFFF",$TID="MyTagCloud",$BW=0,$SA=0,$RTL=0) {
...
$htmlCode = '';
$htmlCode .= '<font face="'.$this->FontType.'">';
$htmlCode .= '<table ID="'.$this->TableIdName.'" NAME="'.$this->TableIdName.'"  bgcolor="'.$this->BackgroundColor.'" cellspacing="'.$this->SpacingPixelBetweenItems.'" cellpadding="'.$this->SpacingPixelBetweenItems.'" border="'.$this->BorderWIdth.'" DIR="'.$this->RightToLeftView.'">';
...
reset($this->KeywordsArray);
		...
		for ($NumofRaws=1;$NumofRaws<=$this->Rows;$NumofRaws++) // Loop over Raws (correlate to Table TR Tag ) 
		{
		$htmlCode .= '<TR>';
		    ...
		    $htmlCode .= '<TD><a href="'.$this->LinkUrlPrefix.$currentKeyLink.'" style="color:'.$this->TextColor.';font-size:'.$TagSize.'">'.$currentKey.'</a></TD>';
