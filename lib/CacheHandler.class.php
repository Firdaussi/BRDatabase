...
class CacheHandler {
    ...
    /**
    *   Class constructor - automatically called when initiating an instance of the CacheHandler 
class.
    *   Retrieve configuration variables to correctly setup the caching environment.
    *   @param string|array $caching_path Absolute path to the configuration file where we can find 
environment related variable. OR array with required variables already pulled from the config 
file.
    */
        ...
        if(is_array($config_path))
            ...
            if(file_exists($config_path)) {
                ...
                if(!isset($config['cache'])) {
                    echo "File $config_path can be reached but there is no variable called 
".'$config["cache"].';
                    exit(-1);
        ...
        foreach($this->required_variables as $var) {
            ...
            if(!isset($config[$var])) {
                echo "Error: $var is missing in either your config file or the config array you 
used.";
                exit(-1);
            } else {
    ...
    *   @param string $name Name of the caching file. Has to be unique since you'll pull your data 
from the cache engine with this variable.
    *   @param mixed $content Can be any type of variable that you want to cache.
    *   @param int $duration Time you want to keep the content cached (in SECONDS). If $duration = 0, 
we'll keep the file cached for ever.
    ...
    *   @param string $name Name of the caching file. Has to be unique since you'll pull your data 
from the cache engine with this variable.
    *   @param int $duration Time you want to keep the content cached. If $duration = 0, we'll keep 
the file cached for ever.
    ...
    */
    ...
    /**
    *   Delete a cache file. Usually not used unless you create a cache file with $duration = 0 and 
want to regenerate the cache.
