...
<?php
  ...
  $hst = new MyTables("history");
  $hst->add_column("dt",             "Date",          5);
  $hst->add_column("magazine",       "Source",       13);
  $hst->add_column("n",              "New",           4);
  $hst->add_column("w",              "Wdth",          4);
  $hst->add_column("a",              "Allc",          4);
  $hst->add_column("m",              "Mods",          4);
  $hst->add_column("c",              "Reb",           4);
  $hst->add_column("n",              "Ren",           4);
  $hst->add_column("d",              "DMUs",          4);
  $hst->add_column("e",              "EMUs",          4);
  $hst->add_column("r",              "RCar",          4);
  $hst->add_column("x",              "Err",           4);
  $hst->add_column("wr",             "WR",            7);
  $hst->add_column("sr",             "SR",            7);
  $hst->add_column("lmr",            "LMR",           7);
  $hst->add_column("er",             "ER",            7);
  $hst->add_column("ne",             "NER",           7);
  $hst->add_column("scr",            "SCR",           7);

  while ($row = mysqli_fetch_array($result))
  {
    $r['dt'] = $row['month'] . "/" . $row['year'];
    $r['magazine'] = $row['magazine'];

    $r['w'] = fn_icon($row['withdrawals']);
    $r['n'] = fn_icon($row['new_builds']);
    $r['a'] = fn_icon($row['reallocs']);
    $r['d'] = fn_icon($row['dmus']);
    $r['r'] = fn_icon($row['rcar']);
    $r['e'] = fn_icon($row['emus']);
    $r['m'] = fn_icon($row['mods']);
    $r['c'] = fn_icon($row['reclassified']);
    $r['n'] = fn_icon($row['renumbered']);
    $r['x'] = fn_icon($row['errata']);

    $r['wr']  = fn_fdate($row['wr_date']);
    $r['sr']  = fn_fdate($row['sr_date']);
    $r['lmr'] = fn_fdate($row['lmr_date']);
    $r['er']  = fn_fdate($row['er_date']);
    $r['ne']  = fn_fdate($row['ner_date']);
    $r['scr'] = fn_fdate($row['scr_date']);
