...
//  echo $file_cache->get_cachefile();
  ...
  $sql = 'select scl1.s_class_id,
          ...
          and    scl2.s_class_id IN (601504, 601505)
          join   s_class sc2
  ...
  $sql3 = 'select sn.company,
           ...
           and    scl.s_class_id IN (601501, 601504, 601505)
  ...
  $sql = 'SELECT s.loco_id lid,
                 ...
                 concat(date_format(s.b_date, "%Y%m%d"), 
                                     lpad(s.loco_id, 7, "0000000"))         AS b_date_fmt,
                 s.w_date,
                 concat(date_format(s.w_date, "%Y%m%d"), 
                                     lpad(s.loco_id, 7, "0000000"))         AS w_date_fmt,
                 ...
                 concat(date_format(s.s_date, "%Y%m%d"), 
                                     lpad(s.loco_id, 7, "0000000"))         AS s_date_fmt,
                 s.preserved,
          ...
          ON     dc2.depot_code = ifnull(s.last_depot, s.last_depot_bak)
          AND    dc2.date_from = (SELECT max(dc2a.date_from)
                                  FROM   ref_depot_codes dc2a
                                  WHERE  dc2a.depot_code = ifnull(s.last_depot, s.last_depot_bak)
          ...
          AND    scl.s_class_id IN (601504, 601505)
          ...
          JOIN (    select scl2.loco_id,
                    ...
                    where  scl2.s_class_id IN (601501, 601504, 601505)
          ) AS F
  ...
  printf("<th width=24%%><strong>Fate</strong></th>\n");
  printf("</tr>\n");
      ...
      if (strcmp($last_loco_id, "0") != 0)
      {
        printf("<tr>\n");
          for ($ncomp = 0; $ncomp < $arct; $ncomp++)
          {
            ...
            fn_ptdelem(60/$arct, $loconum[$ncomp], $fmt);
            $loconum[$ncomp] = "";
          }
