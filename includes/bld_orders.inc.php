<?php
  ...
  if (empty($lot))
  {
    ...
    $tb->sortable();
    $tb->allow_rollover();
    $tb->add_column("lot_number",        "Lot Number",    7);
    $tb->add_column("order_number",      "Order Number",  7);
    ...
    $tb->add_column("wheel_arrangement", "Wheels",        8);
    $tb->add_column("size",              "Number Off",    5);
    $tb->add_column("info",              "Info",         32);

    $sql = 'select if(o.virtual_ind="N", o.order_number, "Unknown")    AS order_number,
                   o.lot_number,
                   ...
                   o.type,
                   o.class_id,
                   coalesce(s.common_name, s.identifier)               AS s_ident,
                   ...
                   coalesce(d.common_name, d.identifier)               AS d_ident,
                   concat("locoqry.php?action=class&type=D&id=",d.d_class_id) AS d_ident_hl,
                   "BR"                                                AS d_company,
                   coalesce(e.common_name, e.identifier)               AS e_ident,
  ...
  else  /* Specific lot number details */
  {
    ...
    $sql = 'SELECT o.*,
                   ...
                   bl.*,
                   bl_acc.bl_name         AS acc_bl_name,
                   ifnull(coalesce(s.common_name, s.identifier),
                          ifnull(coalesce(e.common_name, e.identifier), 
                                 coalesce(d.common_name, d.identifier))) AS identifier
            FROM   ref_orders o
            
            JOIN   ref_builders bl
            ON     bl.bl_code = o.bl_code
            
            LEFT JOIN ref_builders bl_acc
            ON     bl_acc.bl_code = o.acc_bl_code
    ...
    $tb1->add_row("accinfo",           "Administration");
    $tb1->add_row_lwidth(40);
    $tb1->set_align("V");
    $row['order_date'] = fn_fdate($row['order_date']);
    
    if (!empty($row['acc_bl_name']) &&
        !empty($row['acc_order_number']))
    {
      $row['accinfo'] = $row['acc_bl_name'] . " order #" . $row['acc_order_number'];
    }
    ...
    if ($row['otype'] == "S")
    {
      $sql = 'SELECT s.loco_id,
              ...
              UNION
              SELECT s.loco_id,
                     scl.start_date                                AS b_date,
                     date_format(scl.start_date, "%Y%m%d")         AS b_date_fmt,
                     s.w_date,
                     date_format(s.w_date, "%Y%m%d")               AS w_date_fmt,
                     s.works_num,
                     NULL           AS works_num_b,
              ...
              FROM   steam s
              JOIN   ref_orders o
              ON     o.order_id = s.rb_order_id
              JOIN   s_class sc
              ON     sc.s_class_id = o.class_id
              JOIN   s_class_link scl
              ON     s.loco_id = scl.loco_id
              AND    scl.s_class_id = o.class_id
              AND    scl.first_class_flag = "N"
              LEFT JOIN s_nums sn
              ON     sn.loco_id = s.loco_id
              AND    sn.start_date = s.b_date
              LEFT JOIN ref_people p
    ...
    if ($row['otype'] == "D")
    {
      $sql = 'SELECT d.loco_id,
                     ...
                     NULL                                                  AS designer,
                     NULL                                                  AS designer_hl,
                     ifnull(dc.prg_company, ifnull(dc.big4_company, "BR")) AS company,
                     concat("companies.php?page=", ifnull(dc.big4_company, "BR"),
                                                   CASE ifnull(dc.prg_company, "#")
                                                       WHEN "#" THEN ""
                                                   ELSE
                                                       concat("&subpage=", dc.prg_company)
                                                   END) AS company_hl,
