...
/****************************************************************************************/
  ...
  $sql = 'SELECT   dcc.*
          FROM     d_class_codes dcc
          WHERE    dcc.d_class_id = ' .$id. '
          ORDER BY dcc.start_date ASC, d_class_var_id';
  ...
  if ($db->count_select())
  {
    while($dccrow = mysqli_fetch_assoc($result))
    {
      ...
      $dccrow['dcc'.$nx] = $dccrow['d_class_code'];
...
/****************************************************************************************/
  ...
  $tb_basics->add_row("reports",                   "Reports");
...
/****************************************************************************************/
  ...
  $sql = 'SELECT b.bl_name,
          ...
          JOIN  (SELECT DISTINCT dcl.loco_id
                 FROM   d_class_link dcl
                 WHERE  d_class_id = ' . $id . ') AS F
...
/****************************************************************************************/
  ...
  {
    $nr++;
    $line = "<table width=99% frame=\"box\"><tr><td width=50%><strong>Builder</strong></td>";
    $line .=                               "<td width=20%><strong>Order #</strong></td>";
    $line .=                               "<td width=20%><strong>Date</strong></td>";
    ...
    while ($rowb = mysqli_fetch_assoc($result))
    {
      ...
      $line .= "<tr><td width=50%><a href=\"" . $rowb['bl_name_hl'] . "\">" .
                                                $rowb['bl_name']    . "</a>";
      if (strcmp($rowb['bl_code'], "NB") == 0 && !empty($rowb['info']))
        $line .= " (" . $rowb['info'] . ")";
      $line .= "</td>";

      $line .= "    <td width=20%><a href=\"" . $rowb['order_number_hl'] . "\">" .
                                                $rowb['order_number'] . "</a></td>";
      $line .= "    <td width=20%>" . $rowb['order_date'] . "</td>";
      $line .= "    <td width=10%>" . $rowb['size'] . "</td></tr>";
    }
...
/****************************************************************************************/
/* Get any associated relevant reports                                                  */
/****************************************************************************************/

  $sql = 'SELECT *,
                 concat("reports.php?rpopt=", rep_id) AS report_hl
          FROM   d_reports
	  WHERE  d_class_id = ' . $id . '
  ...
  {
    $nr++;
    $line = "<table width=99% frame=none><tr><td width=100%><strong>Title</strong></td></tr>";

    while ($rowb = mysqli_fetch_assoc($result))
    {
      $line .= "<tr><td width=100%><a href=\"" . $rowb['report_hl'] . "\">" .
                                                 $rowb['description']  . "</a></td>";
    }
...
/****************************************************************************************/
  ...
  $tb_dim->add_row("wt_loco_weight",               "Weight");
  $tb_dim->add_row("wt_loco_weight_empty",         "Weight (when empty)");
  ...
  $tb_dim->add_row("str_minimum_curve",            "Minimum Curve");

  $sql = 'SELECT  dcv.d_class_var_id                         AS int_d_class_var_id,
                  dcv.identifier                             AS str_class_type,
                  concat("locoqry.php?action=class&id=", dcv.d_class_id, 
                         "&type=D&page=fleet&var=", dcv.d_class_var_id)
                                                             AS hl_class_type,
                  ...
                  dcv.loco_weight                            AS wt_loco_weight, 
                  dcv.loco_weight_empty                      AS wt_loco_weight_empty, 
                  dcv.wheel_diameter                         AS ft_wheel_diameter, 
                  dcv.pony_wheel_diameter                    AS ft_pony_wheel_diameter, 
                  dcv.carrier_wheel_diameter                 AS ft_carrier_wheel_diameter, 
                  ...
                  dcv.brakes_loco                            AS str_brakes_loco, 
                  dcv.brakes_train                           AS str_brakes_train, 
                  ...
                  dcv.d_class_var_base                       AS ign_var_base,
          ...
          ON     b.bl_code = dcv.bl_code

          WHERE  dcv.d_class_id = ' .$id. '
          ORDER BY dcv.d_class_var_id';
...
//print_r($v); echo "<br />";
  ...
  for ($nx = 0; $nx < $ct; $nx++)
  {
    ...
    // For this variant, if the d_class_var_base is set, use it for the default (if NULL,
...
/****************************************************************************************/
  ...
  $sql = 'SELECT concat("./images/locos/diesel/", i.image) AS image_url,
                 i.photo_date,
                 ic.copyright,
                 ic.copyright_url
