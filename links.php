...
$tb = new MyTables("links");

$sql = 'SELECT lkt.*
        FROM   ref_link_type lkt
	    ORDER BY lkt.link_rank ASC';
...
else
{
  ...
  {
    $sql = 'SELECT lk.*,
                   coalesce(ic.copyright_url, lk.link_site) AS link_site,
                   concat("\"javascript:void(0)\" onClick=\"window.open(\'https://", 
                          coalesce(ic.copyright_url, lk.link_site), "\')\"") 
                                                            AS link_site_hl
                   FROM   ref_links lk
                   LEFT JOIN ref_image_copyright ic
                   ON     ic.ic_id = lk.ic_id
                   WHERE  lk.link_type = "' . $row1['link_type'] . '"
                   ORDER BY link_site, other_source, seq, link_info';

    $res2 = $db->execute($sql);

    $tb->add_caption($row1['link_description']);
    $tb->add_column("link_site",     "Source",  25);
    $tb->add_column("link_info",     "Description", 75);

    while ($row2 = mysqli_fetch_array($res2))
    {
      if ($row2['link_site'] == "")
        $row2['link_site'] = $row2['other_source'];
      $tb->add_data($row2);
    }
